...
#[cfg(not(feature = "std"))]
use crate::heap_alloc::init_heap_alloc;
...
mod c_api_translation {
    ...
    impl<'t> Into<SrcLoc<'t>> for &'t c_api::src_loc {
        fn into(self) -> SrcLoc<'t> {
            unsafe {
                SrcLoc {
                    file: CStr::from_ptr(self.file).to_str().unwrap(),
                    func: CStr::from_ptr(self.func).to_str().unwrap(),
                    lineno: self.lineno as u64,
                }
            }
        }
    }
...
pub fn casemate_config(config: *const ::core::ffi::c_char) -> ::core::ffi::c_int {
    ...
    match config::de::make_config_builder(s) {
        Ok(builder) => configure(builder),
        Err(e) => {
            warn!("failed to parse JSON-encoded config: {:?}", e);
            return -1;
        }
    }
    0
...
pub fn casemate_initialize_model(
    phys: u64,
    size: u64,
    #[cfg_attr(feature = "std", allow(unused_variables))] sm_virt: *mut ::core::ffi::c_void,
    #[cfg_attr(feature = "std", allow(unused_variables))] sm_size: u64,
) -> ::core::ffi::c_int {
    #[cfg(not(feature = "std"))]
